...
class ReplayBuffer(Dataset):
    ...
    def sample_proprio(self):
        ...
        #print(idxs,obses.shape,actions.shape,rewards.shape,next_obses.shape,not_dones.shape)
    ...
    def sample(self):
        ...
        return obses, actions, curr_rewards, rewards, next_obses
    ...
    def sample_spr(self):   # sample batch for auxiliary task
        ...
        # 单独处理 obses/next_obses
        obses = self.obses[idxs]
        next_obses = self.next_obses[idxs]

        B, T, F, S, S = obses.shape
        obses = obses.reshape(B * T, F, S, S)  # (B, jumps+1, 3*3=9, 100, 100)
        next_obses = next_obses.reshape(B * T, F, S, S)

        obses = random_crop(obses, self.image_size) 
        next_obses = random_crop(next_obses, self.image_size) 

        obses = obses.reshape(B, T, F, self.image_size, self.image_size)
        next_obses = next_obses.reshape(B, T, F, self.image_size, self.image_size)

        obses = torch.as_tensor(obses, device=self.device).float()   # (B, jumps+1, 3*3=9, 84, 84)
        next_obses = torch.as_tensor(next_obses, device=self.device).float()
        actions = torch.as_tensor(self.actions[idxs][:, -1, :], device=self.device)
        rewards = torch.as_tensor(self.rewards[idxs][:, -1, :], device=self.device)
        not_dones = torch.as_tensor(self.not_dones[idxs][:, -1, :], device=self.device)   # (B, jumps+1, 1)
        ...
        #print(idxs,obses.shape,actions.shape,rewards.shape,next_obses.shape,not_dones.shape,obses.transpose(0, 1).unsqueeze(3).shape,reward.transpose(0, 1).shape)

        return (spr_samples)
        #return (obses, actions, rewards, next_obses, not_dones, spr_samples)
